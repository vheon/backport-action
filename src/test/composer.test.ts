...
describe("composeComment", () => {
  ...
  describe("of type failed_to_push", () => {
    test("should start with standard introduction", () => {
      expect(composeComment(MessageType.failed_to_push, {})).toStartWith(
        introduction,
      );
    });
    ...
    test("should link to workflow run", () => {
      expect(
        composeComment(MessageType.failed_to_push, {
          run_id: 123,
    ...
    test("should say what happened", () => {
      expect(
        composeComment(MessageType.failed_to_push, { target: "target_branch" }),
